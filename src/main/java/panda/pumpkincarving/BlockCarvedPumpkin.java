...
public class BlockCarvedPumpkin extends BlockHorizontal {
	...
	private static final Predicate<IBlockState> IS_PUMPKIN = new Predicate<IBlockState>() {
		public boolean apply(@Nullable IBlockState state) {
			return state != null && (state.getBlock() == PumpkinCarving.carvedPumpkin
					|| state.getBlock() == PumpkinCarving.carvedPumpkinLit);
		}
	};
	...
	protected BlockCarvedPumpkin() {
		...
		this.setDefaultState(this.blockState.getBaseState().withProperty(FACING, EnumFacing.NORTH).withProperty(FACE,
				Integer.valueOf(0)));
	...
	private void trySpawnGolem(World worldIn, BlockPos pos) {
		...
		if (patternhelper != null) {
			...
			for (EntityPlayerMP entityplayermp : worldIn.getEntitiesWithinAABB(EntityPlayerMP.class,
					entitysnowman.getEntityBoundingBox().grow(5.0D))) {
			...
			if (patternhelper != null) {
				for (int j = 0; j < this.getGolemPattern().getPalmLength(); ++j) {
					for (int k = 0; k < this.getGolemPattern().getThumbLength(); ++k) {
						worldIn.setBlockState(patternhelper.translateOffset(j, k, 0).getPos(),
	...
	public IBlockState getStateForPlacement(World worldIn, BlockPos pos, EnumFacing facing, float hitX, float hitY,
		...
		return this.getDefaultState().withProperty(FACE, meta).withProperty(FACING,
	...
	public IBlockState getStateFromMeta(int meta) {
		return getDefaultState().withProperty(FACE, getActualMeta(meta)).withProperty(FACING,
				EnumFacing.getHorizontal(meta % 4));
	}

	public int getActualMeta(int meta) { // returns the FACE value for given meta.
		if (meta >= 0 && meta <= 3)
			return 0;
